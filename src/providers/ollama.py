"""Ollama provider implementation for LLM classification."""
...
class OllamaProvider(LLMProvider):
    """Ollama provider for spam classification."""
    ...
    async def classify(self, transcript: str) -> ClassificationResult:
        """Classify a transcript using Ollama.
        ...
        try:
            ...
            logger.error("Ollama classification failed: %s", e, exc_info=True)
            return ClassificationResult(
                is_spam=False,
                confidence=0.0,
                reason=f"Ollama classification error: {e}",
                evidence_lines=[],
                full_transcript=transcript,
            )
